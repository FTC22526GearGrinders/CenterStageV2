...
public class RunTrajSequence extends CommandBase {
    ...
    public void initialize() {
        ...
        trajEnded = Thread.currentThread().isInterrupted() || !drive.drive.isBusy();

        if (!trajEnded) endTime.reset();
    ...
    public boolean isFinished() {
        return et.seconds() > 2 && (trajEnded && endTime.seconds() > .5) || et.seconds() > 4;
    }
